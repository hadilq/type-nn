...
static void *soa_new(size_t in, size_t out, size_t k) { return dense_new(in, out, k, 0); }

static const TLayerOps OPS = {
    soa_new, dense_init, dense_fwd_soa, dense_bwd,
...
{
    AltNet h;
    tstack_bind(&h, tstack_open("type-nn-soa", &OPS, in, out));
    return h;
}
