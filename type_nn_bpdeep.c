...
/*
 * type-nn-bpdeep / type-nn-bpdeep-early
 *
...
{
    /* thin: match torch-mlp's 8-wide iris hidden, not a fat 16–32 row */
    size_t h = 8;
    if (in >= 20) h = 12;
    if (h < out) h = out;
...
static void insert_at(PNet *N, size_t idx)
{
    ...
    palloc(&N->layer[idx], dim, dim, 1);
    {
        ...
        double sc = 0.15 / sqrt((double)(L->in ? L->in : 1));
        for (size_t r = 0; r < L->n_or; r++) {
            L->b[r] = ((double)rand() / RAND_MAX * 2.0 - 1.0) * sc;
            for (size_t j = 0; j < L->in; j++)
                L->W[r * L->in + j] = ((double)rand() / RAND_MAX * 2.0 - 1.0) * sc;
        }
    }
    psync(&N->layer[idx]);
...
static void estimate_and_apply(PNet *N)
{
    ...
    /* Under-layered: many params per layer, error still high → another
       thin k=1 at the bottleneck, never a fatter row. */
    if (N->layer[0].L.in < 4) return;
    if (N->tick > 800) {
        remove_idle(N);
    ...
    if (early && N->layer[0].L.in >= 4 && N->depth == 1) {
        insert_at(N, 0);              /* thin k=1 basis */
        insert_at(N, 1);              /* second thin k=1 (the missing layer) */
        insert_at(N, N->depth);       /* thin k=1 readout */
        N->early_done = 1; N->locked = 1;
        return;
    }

    if (N->added_layers >= 3 || N->depth >= 4)
        return;
