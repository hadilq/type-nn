...
/*
 * type-nn-bpsite / type-nn-bpearly
 *
...
static void grow_width(PNet *N)
{
    if (N->depth < 2) return;
    PLayer *H = &N->layer[0];
    if (H->L.k != 1 || H->L.out >= MAX_HID) return;
...
static void estimate_and_apply(PNet *N)
{
    ...
    if (N->added_layers >= 2 || N->tick > 4000) {
        if (later && N->ema > 0.05) grow_width(N);
        else remove_idle(N);
        N->early_done = 1; N->locked = 1;
        return;
    }

    if (N->force_roles && early && N->layer[0].L.in >= 4 && N->depth == 1) {
        insert_at(N, 0);           /* basis in front of the product */
        insert_at(N, N->depth);    /* readout after the product */
        N->early_done = 1; N->locked = 1;
        return;
    }
...
AltNet type_nn_bpearly_open(size_t in, size_t out)
{
    AltNet h = type_nn_bpsite_open(in, out);
    h.impl = "type-nn-bpearly";
    ((PNet *)h.ctx)->force_roles = 1;
    ((PNet *)h.ctx)->name = "type-nn-bpearly";
    return h;
}
