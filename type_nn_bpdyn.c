...
static void estimate_and_apply(PNet *N)
{
    ...
    /* Projection missing: product sitting on wide raw input + error energy. */
    if (in0 >= 4 && !has_k1_front(N) && N->layer[0].ge > 0.06)
        n_add_proj = 1;

    /* Readout missing: product writing a multi-d target + error energy. */
    if (outT >= 2 && !has_k1_readout(N) &&
        N->layer[N->depth - 1].L.k >= 2 &&
        N->layer[N->depth - 1].ge > 0.05)
        n_add_readout = 1;

    for (size_t i = 0; i + 1 < N->depth; i++)
        if (layer_idle(&N->layer[i])) n_remove++;

    /* Apply in priority: structure first, then drop dead maps.
       Never go past the 3-role stack. */
    if (n_add_proj && N->depth < 3) {
        insert_proj(N);
        return;
    }
    if (n_add_readout && N->depth < 4) {
        insert_readout(N);
        return;
    }
    if (n_remove)
